...
export default async function BrandPage(props: PageProps) {
  ...
  const { items: products } = await getPublicProductsAction(1, 100, sort, { 
      brandSlugOrId: brandSlugRaw,
      gender: gf as 'M' | 'W' | 'A',
  });

  return (
    <div className="container mx-auto px-4 py-8">
      <h1 className="text-2xl font-bold mb-6">Brand Products</h1>

      <div className="flex justify-end gap-4 mb-4 text-xs font-medium text-gray-500">
        <Link href={`/brand/${brandSlugRaw}?gf=${gf}&sort=newest`} className={sort === 'newest' ? 'text-black font-bold' : 'hover:text-black'}>최신순</Link>
        <Link href={`/brand/${brandSlugRaw}?gf=${gf}&sort=priceAsc`} className={sort === 'priceAsc' ? 'text-black font-bold' : 'hover:text-black'}>가격낮은순</Link>
        <Link href={`/brand/${brandSlugRaw}?gf=${gf}&sort=priceDesc`} className={sort === 'priceDesc' ? 'text-black font-bold' : 'hover:text-black'}>가격높은순</Link>
      </div>
