...
export default function ProductDisplayEditPage() {
    // Basic Info State
    ...
    const [bottomMoreExposed, setBottomMoreExposed] = useState('hidden');
    ...
    // Display Product List Mock Data
    const products = [
        { id: 1, name: '[Malbon Golf] 여성 니트에리 경량 다운베스트_WHITE (WOMAN)', price: '459,000원', supplier: '나이아인터내셔널', stock: '∞', status: '정상', pc: '노출함', mobile: '노출함', tags: [], image: '/placeholder.png' },
        { id: 2, name: '미니멀 블랙 프레임', price: '19,800원', supplier: '나이아인터내셔널', stock: '∞', status: '정상', pc: '노출함', mobile: '노출함', tags: ['NEW', '자체제작'], image: '/placeholder.png' },
        { id: 3, name: '투명 빈티지 클래식', price: '32,040원', supplier: '나이아인터내셔널', stock: '∞', status: '정상', pc: '노출함', mobile: '노출함', tags: ['최다판매', 'EVENT'], image: '/placeholder.png' },
        { id: 4, name: '크림색 캣아이 선글라스', price: '16,200원', supplier: '나이아인터내셔널', stock: '∞', status: '정상', pc: '노출함', mobile: '노출함', tags: [], image: '/placeholder.png' },
        { id: 5, name: '투명 실버 선글라스', price: '13,500원', supplier: '나이아인터내셔널', stock: '∞', status: '정상', pc: '노출함', mobile: '노출함', tags: ['BEST', 'EVENT'], image: '/placeholder.png' },
        { id: 6, name: '스퀘어 크리스탈 루시드', price: '28,800원', supplier: '나이아인터내셔널', stock: '∞', status: '정상', pc: '노출함', mobile: '노출함', tags: ['주문폭주'], image: '/placeholder.png' },
        { id: 7, name: '클래식 블랙 라운드 선글라스', price: '18,000원', supplier: '나이아인터내셔널', stock: '∞', status: '정상', pc: '노출함', mobile: '노출함', tags: ['당일발송'], image: '/placeholder.png' },
        { id: 8, name: '블랙 스퀘어 선글라스', price: '13,500원', supplier: '나이아인터내셔널', stock: '∞', status: '정상', pc: '노출함', mobile: '노출함', tags: ['추천', 'SALE'], image: '/placeholder.png' },
        { id: 9, name: '오렌지 렌즈 패셔너블 선글라스', price: '27,000원', supplier: '나이아인터내셔널', stock: '∞', status: '정상', pc: '노출함', mobile: '노출함', tags: ['NEW'], image: '/placeholder.png' },
    ];
    ...
    return (
        <div className="p-6 space-y-6 bg-white min-h-screen font-sans text-sm pb-24">
             ...
             <div className="flex items-center justify-between pb-4 border-b border-black">
                <h1 className="text-2xl font-bold text-gray-900">메인페이지 분류 수정</h1>
            ...
            <div className="space-y-4">
                    ...
                    <div className="flex border-b border-gray-200">
                        ...
                        <div className="flex-1 p-3 flex items-center gap-2 border-r border-gray-200">
                            <div className="relative w-64">
                                ...
                                <span className="absolute right-2 top-1/2 -translate-y-1/2 text-xs text-red-500 font-bold">12 / 30</span>
                            </div>
                        </div>
                         <div className="w-40 bg-gray-50 p-3 pl-4 font-bold text-gray-700 flex items-center gap-1">
                    ...
                    <div className="flex border-b border-gray-200">
                     ...
                     <div className="flex border-b border-gray-200">
                         ...
                         <div className="flex-1 p-3 flex items-center gap-2 border-r border-gray-200">
                            ...
                            <Button className="bg-[#6b7280] hover:bg-[#6b7280]/90 text-white h-8 text-xs rounded-sm" onClick={() => window.location.href='/admin/products/main-display/theme/register'}>테마 등록</Button>
                         </div>
                    ...
                    <div className="flex border-b border-gray-200">
                        ...
                        <div className="flex-1 p-3 flex items-center gap-6">
                            <RadioGroup value={bottomMoreExposed} onValueChange={setBottomMoreExposed} className="flex gap-4">
                                <div className="flex items-center space-x-2">
                                    <RadioGroupItem value="exposed" id="bottom-exposed" />
                                    <Label htmlFor="bottom-exposed">노출함</Label>
                                </div>
                                <div className="flex items-center space-x-2">
                                    <RadioGroupItem value="hidden" id="bottom-hidden" className="text-red-500 border-red-500" />
                 ...
                 <div className="border border-gray-300">
                     <div className="flex border-b border-gray-200">
                        <div className="w-40 bg-gray-50 p-3 pl-4 font-bold text-gray-700">테마명</div>
                         <div className="flex-1 p-3 flex items-center gap-2">
                            상단 상품진열 <Button variant="outline" className="h-6 text-xs bg-white text-gray-600 border-gray-300 px-2" onClick={() => window.location.href='/admin/products/main-display/theme/register'}>수정</Button>
                         </div>
                     </div>
                     <div className="flex border-b border-gray-200">
             ...
             <div className="space-y-4 pt-4">
                 ...
                 <div className="border-t-2 border-gray-600 border-b border-gray-300">
                    <Table>
                        <TableHeader>
                            <TableRow className="bg-[#A4A4A4]/40 hover:bg-[#A4A4A4]/40 text-xs text-center font-bold text-gray-700 h-10 whitespace-nowrap">
                                <TableHead className="w-10 text-center p-0"><Checkbox className="translate-y-[2px]" /></TableHead>
                                <TableHead className="text-center w-16">진열순서</TableHead>
                                <TableHead className="text-center w-16">이미지</TableHead>
                                <TableHead className="text-center">상품명</TableHead>
                                <TableHead className="text-center w-24">판매가</TableHead>
                                   ...
                                   <TableCell className="text-left pl-4">
                                       ...
                                       <div className="flex gap-1 mt-1">
                                            {product.tags.map(tag => (
                                                <span key={tag} className={`px-1 py-[1px] text-[10px] text-white rounded-sm font-bold ${
                                                    tag === 'NEW' ? 'bg-black' :
                                                    tag === '자체제작' ? 'bg-black' :
                                                    tag === '최다판매' ? 'bg-gray-400' :
                                                    tag === 'EVENT' ? 'bg-[#b45309]' :
                                                    tag === 'BEST' ? 'bg-gray-400' :
                                                    tag === '주문폭주' ? 'bg-[#ea580c]' :
                                                    tag === '당일발송' ? 'bg-[#ca8a04]' :
                                                    tag === '추천' ? 'bg-black' :
                                                    tag === 'SALE' ? 'bg-[#dc2626]' : 'bg-gray-400'
                                                }`}>
                                                    {tag}
                                                </span>
                                            ))}
