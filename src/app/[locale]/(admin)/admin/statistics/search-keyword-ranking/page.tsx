...
export default function SearchKeywordRankingPage() {
  return (
    <div className="p-6 bg-white min-h-screen font-sans text-xs pb-24 relative">
      {/* Header */}
      <div className="flex items-center justify-between pb-4 border-b-2 border-gray-800 mb-6">
        <h1 className="text-2xl font-bold text-gray-900 leading-none mt-2">검색어순위 분석</h1>
      </div>

       <div className="flex items-center gap-1 mb-2">
         <h2 className="text-base font-bold text-gray-700">검색어 순위 검색</h2>
      ...
      <div className="border-t border-gray-400 mb-6">
        ...
        <div className="flex border-b border-gray-200">
          ...
          <div className="flex-1 flex items-stretch">
             <div className="p-3 flex items-center border-r border-gray-200" style={{ width: '400px' }}>
                <Input className="w-full h-8 text-xs border-gray-300 rounded-[4px]" />
             </div>
             <div className="w-40 bg-gray-50 p-3 flex items-center font-bold text-gray-700 justify-center border-r border-gray-200">
                검색범위
             </div>
             <div className="flex-1 p-3 flex items-center">
                <RadioGroup defaultValue="all" className="flex items-center gap-6">
                    <div className="flex items-center gap-2">
                        <RadioGroupItem value="all" id="scope-all" className="text-red-500 border-gray-300" />
                        <Label htmlFor="scope-all" className="font-normal cursor-pointer">전체</Label>
                    </div>
                    <div className="flex items-center gap-2">
                        <RadioGroupItem value="product" id="scope-product" className="text-red-500 border-gray-300" />
                        <Label htmlFor="scope-product" className="font-normal cursor-pointer">상품명</Label>
                    </div>
                </RadioGroup>
             </div>
          </div>
      ...
      {/* Results Header */}
      <div className="flex items-center justify-between mb-2 p-3 bg-[#f5f5f5] border border-gray-200 rounded-[2px]">
        <div className="flex items-center gap-2">
            <span className="text-gray-600 font-normal">(검색결과 <span className="text-[#FF424D]">0</span> 개)</span>
            <span className="text-gray-600 font-normal">데이터 노출형태 :</span>
            <Select defaultValue="integrated">
              <SelectTrigger className="w-32 h-7 text-xs border-gray-300 bg-white">
                <SelectValue placeholder="통합" />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="integrated">통합</SelectItem>
              </SelectContent>
            </Select>
        </div>
           ...
           <table className="w-full text-center border-collapse text-xs">
            <thead>
               <tr className="bg-[#f5f5f5] text-gray-700 h-10 border-b border-gray-300 font-semibold">
                <th className="border-r border-gray-300 w-20">순위</th>
                <th className="border-r border-gray-300">검색어</th>
                <th className="border-r border-gray-300 w-1/4">검색수</th>
                <th className="w-1/4">비율</th>
               </tr>
            </thead>
            <tbody>
              <tr>
                <td colSpan={4} className="h-40 text-center text-gray-500 font-bold">
                  데이터가 존재하지 않습니다.
                </td>
              </tr>
            </tbody>
