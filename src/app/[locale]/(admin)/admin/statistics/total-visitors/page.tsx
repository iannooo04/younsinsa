...
export default function TotalVisitorsPage() {
  const visitorsData = [
    { date: "2026-01-04", total: 0, new: 0, returning: 0, visits: 0 },
    { date: "2026-01-05", total: 3, new: 2, returning: 1, visits: 3 },
    { date: "2026-01-06", total: 3, new: 1, returning: 2, visits: 3 },
    { date: "2026-01-07", total: 1, new: 0, returning: 1, visits: 1 },
    { date: "2026-01-08", total: 1, new: 0, returning: 1, visits: 1 },
    { date: "2026-01-09", total: 0, new: 0, returning: 0, visits: 0 },
    { date: "2026-01-10", total: 0, new: 0, returning: 0, visits: 0 },
    { date: "2026-01-11", total: 0, new: 0, returning: 0, visits: 0 },
  ];
  ...
  return (
    <div className="p-6 bg-white min-h-screen font-sans text-xs pb-24 relative">
      ...
      <div className="flex items-center justify-between pb-4 border-b-2 border-gray-800 mb-6">
        <h1 className="text-2xl font-bold text-gray-900 leading-none mt-2">전체 방문자 수</h1>
      </div>
      ...
      <div className="bg-white border border-gray-100 rounded-[20px] p-8 mb-8 shadow-sm">
        ...
        <div className="grid grid-cols-6 gap-4">
          <SummaryCard label="전체 방문자 수" value="8" active />
          <SummaryCard label="신규 방문자 수" value="3" />
          <SummaryCard label="재방문자 수" value="5" />
          <SummaryCard label="방문 횟수" value="8" />
          <SummaryCard label="최대 방문자 수" value="3" sub="2026-01-05" />
          <SummaryCard label="최소 방문자 수" value="1" sub="2026-01-07" />
        </div>
      </div>
      ...
      <div className="bg-white border border-gray-100 rounded-[20px] p-12 mb-8 shadow-sm">
        ...
        <div className="mt-8 flex justify-center gap-6">
          <LegendItem color="#FF6358" label="전체 방문자 수" />
          <LegendItem color="#97E064" label="신규 방문자 수" />
          <LegendItem color="#6ED3E1" label="재방문자 수" />
          <LegendItem color="#B465FF" label="방문 횟수" />
        </div>
      </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-[#f8fafc] h-10 border-b border-gray-100 text-gray-600 font-bold">
                ...
                <th className="px-6 border-r border-gray-100">신규 방문자 수</th>
                <th className="px-6 border-r border-gray-100">재방문자 수</th>
                <th className="px-6">방문 횟수</th>
              </tr>
            </thead>
            <tbody>
              {visitorsData.map((row, idx) => (
                <tr key={idx} className="h-10 border-b border-gray-50 text-gray-700 hover:bg-gray-50/50">
                  ...
                  <td className="px-6 border-r border-gray-50 font-normal">{row.total}</td>
                  <td className="px-6 border-r border-gray-50 font-normal">{row.new}</td>
                  <td className="px-6 border-r border-gray-50 font-normal">{row.returning}</td>
                  <td className="px-6 font-normal">{row.visits}</td>
                </tr>
              ))}
            </tbody>
          </table>
...
function LineChartMock() {
  return (
    <svg width="100%" height="100%" viewBox="0 0 1000 200" preserveAspectRatio="none">
      ...
      {/* Visit Count (Purple) */}
      <path d="M 50 180 L 178.5 20 L 307 20 L 435.5 126.6 L 564 126.6 L 692.5 180 L 821 180 L 950 180" fill="none" stroke="#B465FF" strokeWidth="1.5" />
      
      {/* New Visitors (Green) */}
      <path d="M 50 180 L 178.5 73.3 L 307 126.6 L 435.5 180 L 564 180 L 692.5 180 L 821 180 L 950 180" fill="none" stroke="#97E064" strokeWidth="1.5" />
      
      {/* Returning Visitors (Cyan) */}
      <path d="M 50 180 L 178.5 126.6 L 307 73.3 L 435.5 126.6 L 564 126.6 L 692.5 180 L 821 180 L 950 180" fill="none" stroke="#6ED3E1" strokeWidth="1.5" />
      
      {/* Total Visitors (Red - hidden behind purple mostly in this mock) */}
      <path d="M 50 180 L 178.5 20 L 307 20 L 435.5 126.6 L 564 126.6 L 692.5 180 L 821 180 L 950 180" fill="none" stroke="#FF6358" strokeWidth="1.5" strokeDasharray="4" />
    </svg>
  );
}
