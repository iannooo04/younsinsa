...
export default function DepositAnalysisPage() {
  const [activeTab, setActiveTab] = useState("daily");

  const detailData = [
    { date: "2026-01-05", remain: 0, giveCount: 0, giveAmount: 0, useCount: 0, useAmount: 0 },
    { date: "2026-01-06", remain: 0, giveCount: 0, giveAmount: 0, useCount: 0, useAmount: 0 },
    { date: "2026-01-07", remain: 0, giveCount: 0, giveAmount: 0, useCount: 0, useAmount: 0 },
    { date: "2026-01-08", remain: 0, giveCount: 0, giveAmount: 0, useCount: 0, useAmount: 0 },
    { date: "2026-01-09", remain: 0, giveCount: 0, giveAmount: 0, useCount: 0, useAmount: 0 },
    { date: "2026-01-10", remain: 0, giveCount: 0, giveAmount: 0, useCount: 0, useAmount: 0 },
    { date: "2026-01-11", remain: 0, giveCount: 0, giveAmount: 0, useCount: 0, useAmount: 0 },
  ];
  ...
  return (
    <div className="p-6 bg-white min-h-screen font-sans text-xs pb-24 relative">
      ...
      <div className="flex items-center justify-between pb-4 border-b-2 border-gray-800 mb-6">
        <h1 className="text-2xl font-bold text-gray-900 leading-none mt-2">회원 예치금 분석</h1>
      </div>
      ...
      <div className="border border-gray-200 mb-8 overflow-hidden rounded-[4px]">
        <div className="bg-[#f8fafc] px-4 py-2 border-b border-gray-200 flex items-center gap-1 font-bold text-gray-700">
          예치금 검색 <HelpCircle className="w-3.5 h-3.5 text-gray-400" />
        </div>
        <div className="p-0">
      ...
      <div className="mb-4">
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
          <TabsList className="bg-transparent border-b border-gray-300 w-full justify-start rounded-none h-10 p-0 overflow-hidden">
            <TabsTrigger value="daily" className="px-6 h-full data-[state=active]:bg-white data-[state=active]:border-2 data-[state=active]:border-b-0 data-[state=active]:border-gray-300 rounded-none bg-[#f8fafc] border border-gray-200 border-b-0 text-gray-600 font-normal transition-none mr-[-1px]">일별 예치금 현황</TabsTrigger>
            <TabsTrigger value="monthly" className="px-6 h-full border border-gray-200 border-b-0 rounded-none bg-[#f8fafc] text-gray-600 font-normal transition-none">월별 예치금 현황</TabsTrigger>
          </TabsList>
        </Tabs>
      </div>
        ...
        <table className="w-full text-center border-collapse text-[10px]">
          <thead>
            <tr className="bg-[#f8fafc] text-gray-600 h-10 border-b border-gray-200">
              <th className="px-2 border-r border-gray-200 font-normal bg-[#eaf1f7] text-gray-700 w-1/5">총 잔여 예치금</th>
              <th className="px-2 border-r border-gray-200 font-normal w-1/5">총 지급건수</th>
              <th className="px-2 border-r border-gray-200 font-normal w-1/5">총 지급금액</th>
              <th className="px-2 border-r border-gray-200 font-normal w-1/5">총 사용건수</th>
              <th className="px-2 font-normal w-1/5">총 사용금액</th>
            </tr>
          </thead>
          <tbody>
            <tr className="h-14 text-[13px] text-gray-900 border-b border-gray-200">
              ...
              <td className="px-2 border-r border-gray-100 font-normal">0</td>
              <td className="px-2 border-r border-gray-100 font-normal">0</td>
              <td className="px-2 border-r border-gray-100 font-normal">0</td>
              <td className="px-2 font-normal">0</td>
            </tr>
          </tbody>
        </table>
      </div>
      ...
      <div className="border border-gray-300 p-8 mb-8 relative">
        <div className="h-[300px] w-full border-l border-b border-gray-300 relative">
          ...
          {/* Baseline Graph Line */}
          <div className="absolute bottom-0 left-0 w-full h-[1.5px] bg-[#F1975A]" />
        </div>
          ...
          <table className="w-full text-center border-collapse text-gray-600">
            <thead>
              <tr className="bg-[#f8fafc] text-gray-700 font-bold h-10 border-b border-gray-300 text-[11px]">
                ...
                <th className="px-2 border-r border-gray-200">잔여 예치금</th>
            ...
            <tbody>
              {detailData.map((row, idx) => (
                <tr key={idx} className="h-10 border-b border-gray-200 font-normal">
                   ...
                   <td className="px-2">{row.useAmount}</td>
                </tr>
              ))}
            </tbody>
          </table>
