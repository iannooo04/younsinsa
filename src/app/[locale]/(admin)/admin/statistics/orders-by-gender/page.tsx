...
export default function OrdersByGenderPage() {
  const [activeTab, setActiveTab] = useState("daily");

  const days = ["2026-01-05", "2026-01-06", "2026-01-07", "2026-01-08", "2026-01-09", "2026-01-10", "2026-01-11"];

  const genderGroups = ["남자", "여자", "성별 미확인"];

  return (
    <div className="p-6 bg-white min-h-screen font-sans text-xs pb-24 relative">
      {/* Header */}
      <h1 className="text-2xl font-bold text-gray-900 mb-4 pb-4 border-b-2 border-gray-800">성별 주문통계</h1>
      ...
      <div className="mb-12">
           ...
           <table className="w-full text-center border-collapse text-xs min-w-[1400px]">
            <thead>
               <tr className="bg-[#f5f5f5] text-gray-700 h-10 border-b border-gray-300 font-semibold text-center border-t border-t-gray-400">
                ...
                {genderGroups.map((gender, idx) => (
                    <th key={idx} colSpan={4} className="border-r border-gray-300">{gender}</th>
                ))}
               </tr>
               <tr className="bg-[#f5f5f5] text-gray-700 h-8 border-b border-gray-300 font-semibold text-center">
                    {genderGroups.map((_, idx) => (
       ...
       <div className="border-t border-gray-300 pt-6">
        ...
        <div className="space-y-6 text-xs text-gray-600">
          ...
          <div>
            <h4 className="font-bold text-gray-800 mb-1">[통계 정보] 성별의 기준은 무엇인가요?</h4>
            <ul className="list-none pl-1 space-y-1">
              <li><span className="mr-1">·</span>성별 정보는 구매자가 회원가입 시 입력한 "성별" 정보를 기준으로 집계되며, 가입 시 입력한 정보가 없는 경우 "성별 미확인" 주문으로 집계됩니다.</li>
