...
export default function SalesByRegionPage() {
  ...
  // Grid layout for regions summary
  const gridRows = [
      regions.slice(0, 6),
      regions.slice(6, 12),
      regions.slice(12, 17)
  ...
  return (
    <div className="p-6 bg-white min-h-screen font-sans text-xs pb-24 relative">
      ...
      <div className="flex items-center justify-between pb-4 border-b-2 border-gray-800 mb-6">
        <h1 className="text-2xl font-bold text-gray-900 leading-none mt-2">지역별 매출통계</h1>
      ...
      <div className="flex border border-gray-300 mb-8 bg-[#F3F6F9]/50">
          ...
          <div className="flex-1 flex flex-col divide-y divide-gray-300">
              <div className="grid grid-cols-6 divide-x divide-gray-300">
                  {gridRows[0].map(region => <RegionSummary key={region} name={region} value="0원" />)}
              </div>
              <div className="grid grid-cols-6 divide-x divide-gray-300">
                  {gridRows[1].map(region => <RegionSummary key={region} name={region} value="0원" />)}
              </div>
              <div className="grid grid-cols-6 divide-x divide-gray-300">
                   {/* Last row has 5 items but needs to align with 6 columns. Flexbox or grid with placeholders works. 
                       The screenshot shows empty space for the last cell.
                   */}
                  {gridRows[2].map(region => <RegionSummary key={region} name={region} value="0원" />)}
      ...
      <div className="mb-12">
        ...
        <div className="border border-gray-300 border-t-0 overflow-x-auto">
           <table className="w-full text-center border-collapse text-xs min-w-[1600px] table-fixed">
            <thead>
               <tr className="bg-[#f5f5f5] text-gray-700 h-10 border-b border-gray-300 font-semibold text-center border-t border-t-gray-400">
                ...
                {regions.map((region, idx) => (
                    <th key={idx} className="border-r border-gray-300 w-24">{region}</th>
                ))}
               </tr>
            </thead>
            <tbody>
                    ...
                    <React.Fragment key={date}>
                        {/* PC Row */}
                         <tr className="h-8 border-b border-gray-200 hover:bg-gray-50 text-gray-600">
                            ...
                            {regions.map((_, idx) => <td key={idx} className="border-r border-gray-200">0</td>)}
                        </tr>
                        {/* Mobile Row */}
                        <tr className="h-8 border-b border-gray-200 hover:bg-gray-50 text-gray-600">
...
function RegionSummary({ name, value }: { name: string; value: string }) {
    return (
        <div className="flex flex-col items-center justify-center p-4 bg-white min-h-[80px]">
            <h3 className="text-gray-600 mb-4">{name}</h3>
