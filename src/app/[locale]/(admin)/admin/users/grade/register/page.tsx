...
export default function MemberGradeRegisterPage() {
    ...
    const [evalException, setEvalException] = useState("unused");
        ...
        <div className="p-6 bg-white min-h-screen font-sans text-xs pb-24 relative">
            ...
            <div className="flex items-center justify-between pb-4 border-b-2 border-gray-800 mb-6">
                <div className="flex items-center gap-2">
                    <span className="text-gray-500">회원 &gt; 회원 관리 &gt; 회원 등급 등록</span>
                </div>
            </div>
            <div className="flex items-center justify-between mb-6">
                 <h1 className="text-2xl font-bold text-gray-900">회원 등급 등록</h1>
                ...
                <div className="border-t border-gray-400 border-b border-gray-200">
                    ...
                    <div className="flex border-b border-gray-200">
                        ...
                        <div className="flex-1 p-4 bg-white flex items-center gap-2">
                            ...
                            />
                            <span className="text-gray-400 text-xs">0 / 25</span>
                                ...
                                <div className="flex items-start space-x-2">
                                     ...
                                     <div className="flex flex-col gap-2">
                                         ...
                                         <div className="flex gap-2 items-center">
                                             ...
                                             {/* Mock icons */}
                                             {[1, 2, 3, 4, 5].map(i => (
                                                 <div key={i} className="w-6 h-6 border border-gray-200 rounded-sm bg-green-100"></div> 
                                             ))}
                                         </div>
                                     </div>
                                </div>
                                <div className="flex items-start space-x-2">
                                    ...
                                    <div className="flex flex-col gap-1">
                                         ...
                                         <div className="flex gap-1">
                                             <Button 
                                                className="h-6 px-2 text-[11px] bg-[#333333] text-white rounded-[2px] hover:bg-[#222222]"
                                                onClick={() => directIconInputRef.current?.click()}
                                             >
                                                찾아보기
                                             </Button>
                                             <Input value={iconFileName} disabled className="w-40 h-6 bg-gray-100 border-gray-300 rounded-[2px] text-[11px]" />
                                             <input 
                                                type="file" 
                                                ref={directIconInputRef} 
                                                className="hidden" 
                                                accept="image/gif, image/jpeg, image/png" 
                                                onChange={(e) => handleFileChange(e, setIconFileName)}
                                             />
                                         </div>
                                ...
                                <div className="flex items-start space-x-2">
                                     ...
                                     <div className="flex flex-col gap-2">
                                         <Label htmlFor="img-use" className="font-normal text-xs cursor-pointer">기본 등급 이미지</Label>
                                          <div className="flex gap-2 items-center">
                                             <div className="w-16 h-16 border border-gray-300 rounded-[2px] flex items-center justify-center bg-gray-50 text-gray-300">
                                                 <Camera size={24} />
                                             </div>
                                             <Button className="h-6 px-2 text-[11px] bg-[#333333] text-white rounded-[2px] hover:bg-[#222222]">삭제</Button>
                                              {[1, 2, 3, 4, 5].map(i => (
                                                 <div key={i} className="w-12 h-12 border border-gray-200 rounded-full bg-[#A67C4D] flex items-center justify-center text-white">
                                                     Logo
                                                 </div> 
                                             ))}
                                         </div>
                                 ...
                                 <div className="flex items-start space-x-2">
                                    ...
                                    <div className="flex flex-col gap-1">
                                         ...
                                         <div className="flex gap-1">
                                             ...
                                             <Input value={imageFileName} disabled className="w-40 h-6 bg-gray-100 border-gray-300 rounded-[2px] text-[11px]" />
                                             <input 
                                                type="file" 
                                                ref={directImageInputRef} 
                                                className="hidden" 
                                                accept="image/gif, image/jpeg, image/png" 
                                                onChange={(e) => handleFileChange(e, setImageFileName)}
                                             />
                                         </div>
                                         <p className="text-[11px] text-[#888888] flex items-center gap-1">
                                             <span className="bg-[#555] text-white w-3 h-3 text-[9px] flex items-center justify-center rounded-[2px]">!</span>
                    ...
                    {/* Grade Eval Criteria */}
                     <div className="flex border-b border-gray-200">
                        ...
                        <div className="flex-1 p-4 bg-white flex items-center text-gray-500 text-[11px]">
                             <span className="bg-[#555] text-white w-3 h-3 text-[9px] flex items-center justify-center rounded-[2px] mr-1">!</span>
                             [회원&gt;회원관리&gt;회원등급 평가방법 설정]에서 설정한 기준에 따름 (회원등급 등록 이후 설정 가능)
                        </div>
                    </div>
                        ...
                        <div className="flex-1 p-4 bg-white flex flex-col gap-2">
                             <div className="flex gap-4">
                                 <div className="flex items-center gap-1">
                                     <Checkbox id="pay-bank" className="border-gray-300 rounded-[2px] text-[#FF424D]" />
                                     <Label htmlFor="pay-bank" className="font-normal text-xs cursor-pointer">무통장</Label>
                                 </div>
                                  <div className="flex items-center gap-1">
                                     <Checkbox id="pay-pg" className="border-gray-300 rounded-[2px] text-[#FF424D]" />
                                     <Label htmlFor="pay-pg" className="font-normal text-xs cursor-pointer">PG결제수단</Label>
                                 </div>
                                  <div className="flex items-center gap-1">
                                     <Checkbox id="pay-mileage" className="border-gray-300 rounded-[2px] text-[#FF424D]" />
                                     <Label htmlFor="pay-mileage" className="font-normal text-xs cursor-pointer">마일리지 사용</Label>
                                 </div>
                                  <div className="flex items-center gap-1">
                                     <Checkbox id="pay-deposit" className="border-gray-300 rounded-[2px] text-[#FF424D]" />
                    ...
                    {/* Additional Discount */}
                     <div className="flex border-b border-gray-200">
                        ...
                        <div className="flex-1 p-4 bg-white flex items-center gap-2 text-xs">
                             ...
                             <span>구매금액이</span>
                             <Input className="w-24 h-7 text-xs border-gray-300 rounded-[2px]" />
                     ...
                     <div className="flex border-b border-gray-200">
                        <div className="w-48 bg-[#FBFBFB] p-4 flex flex-col justify-center font-bold text-gray-700 text-xs">
                         ...
                         <div className="flex-1 p-4 bg-white flex items-center gap-2 text-xs">
                             ...
                             <Input className="w-24 h-7 text-xs border-gray-300 rounded-[2px]" />
                             <span>원 이상인 경우</span>
                              <Input className="w-16 h-7 text-xs border-gray-300 rounded-[2px]" />
                             <span>% 추가적립</span>
                        </div>
                    </div>
                    <div className="flex border-b border-gray-200">
