  ...
  const fetchOrders = useCallback(async () => {
      ...
      try {
          const res = await getOrdersAction({
              page,
              limit,
              status: [OrderStatus.CANCEL_REQUEST, OrderStatus.CANCEL_COMPLETE],
              keyword,
              searchType,
  ...
  return (
    <div className="p-6 bg-white min-h-screen font-sans text-xs pb-24 relative">
      <div className="flex items-center justify-between pb-4 border-b border-gray-400 mb-6">
        <h1 className="text-2xl font-bold text-gray-900">취소 리스트</h1>
      </div>
            ...
            <div className="flex items-center text-xs border-b border-gray-200">
             ...
             <div className="flex items-center text-xs">
                ...
                <div className="flex-1 p-3 flex items-center gap-2">
                     <Select defaultValue="cancel_complete_date">
                        <SelectTrigger className="w-24 h-7 text-[11px] border-gray-300">
                            <SelectValue placeholder="취소완료일" />
                        </SelectTrigger>
                        <SelectContent>
          ...
          <table className="w-full text-xs text-center border-collapse table-fixed min-w-[2000px]">
              ...
              <thead className="bg-[#BDBDBD] text-white font-normal">
                  <tr className="h-10">
                      ...
                      <th className="border-r border-[#CDCDCD] break-keep leading-tight px-1">취소신청일<br/>(취소접수일)</th>
                      <th className="border-r border-[#CDCDCD]">취소완료일시</th>
                    ...
                    <tr>
                      ...
                      orders.map((order, idx) => (
                          <tr key={order.id} className="border-b border-gray-200 hover:bg-gray-50 h-8">
                               ...
                               <td className="border-r border-[#CDCDCD]">{order.statusUpdatedAt ? format(new Date(order.statusUpdatedAt), "yyyy-MM-dd HH:mm") : '-'}</td>
                               <td className="border-r border-[#CDCDCD]">{order.status === OrderStatus.CANCEL_COMPLETE ? (order.statusUpdatedAt ? format(new Date(order.statusUpdatedAt), "yyyy-MM-dd HH:mm") : '-') : '-'}</td>
                               ...
                               <td className="border-r border-[#CDCDCD] text-red-500 font-bold">{order.status === OrderStatus.CANCEL_COMPLETE ? '취소완료' : '취소신청'}</td>
                               <td className="border-r border-[#CDCDCD]">-</td>
                               <td className="border-r border-[#CDCDCD]">-</td>
                               <td className="">PC</td>
                          </tr>
              ...
              <div className="space-y-4">
                   <div>
                       ...
                       <div className="pl-0 text-xs text-gray-500 space-y-1">
                           <p>· 취소 리스트의 주문상태는 "기본설정{">"}주문정책{">"}주문상태 설정" 메뉴 내 "취소 상태 설정" 항목에서 운영자가 추가할 수 있으며, 관리자페이지와 쇼핑몰페이지에 노출될 주문상태명을 각각 수정할 수 있습니다.</p>
                  ...
                  <div>
                       ...
                       <div className="pl-0 text-xs text-gray-500 space-y-1">
                           <p>· 취소 리스트 내 주문은 상태변경 및 입금대기 상태로 원복이 불가능하며, 삭제처리만 가능합니다.</p>
                           <p className="text-red-500 font-bold">· 삭제된 주문 정보는 복구가 불가능하므로 신중하게 해야합니다.</p>
                       </div>
                  </div>

                  <div>
                       <h3 className="font-bold text-gray-800 mb-1 text-xs">[주문 정보] 자동취소란 무엇인가요?</h3>
                       <div className="pl-0 text-xs text-gray-500 space-y-1">
                           <p>· 자동취소란 구매자가 무통장 입금 또는 가상계좌 등을 이용 주문완료 후 일정 기간동안 결제를 하지않은 주문을 시스템이 취소한 상태입니다.</p>
                           <p>· 미입금 주문 자동취소 기간은 "기본설정{">"}주문정책{">"}주문상태 설정"의 "취소 상태 설정" 항목에서 운영자가 설정할 수 있습니다.</p>
                       </div>
                  </div>
              </div>
           </div>
