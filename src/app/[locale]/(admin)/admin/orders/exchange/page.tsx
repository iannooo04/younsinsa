  ...
  const fetchOrders = useCallback(async () => {
      ...
      try {
          const res = await getOrdersAction({
              page,
              limit,
              status: [OrderStatus.EXCHANGE_REQUEST, OrderStatus.EXCHANGE_COMPLETE],
              keyword,
              searchType,
          ...
          <div className="flex items-center gap-1 text-xs">
              ...
              <Select defaultValue="status">
                  ...
                  <SelectContent>
                      ...
                      <SelectItem value="exchange_request">교환접수</SelectItem>
                      <SelectItem value="returning">반송중</SelectItem>
                      <SelectItem value="reshipping">재배송중</SelectItem>
                      <SelectItem value="exchange_hold">교환보류</SelectItem>
                      <SelectItem value="exchange_complete">교환완료</SelectItem>
                  </SelectContent>
              </Select>
                    ...
                    <tr>
                      ...
                      orders.map((order, idx) => (
                          <tr key={order.id} className="border-b border-gray-200 hover:bg-gray-50 h-8">
                               ...
                               <td className="border-r border-[#CDCDCD]">{order.statusUpdatedAt && order.status === OrderStatus.EXCHANGE_REQUEST ? format(new Date(order.statusUpdatedAt), "yyyy-MM-dd HH:mm") : '-'}</td>
                               <td className="border-r border-[#CDCDCD]">{order.status === OrderStatus.EXCHANGE_COMPLETE ? (order.statusUpdatedAt ? format(new Date(order.statusUpdatedAt), "yyyy-MM-dd HH:mm") : '-') : '-'}</td>
                               ...
                               <td className="border-r border-[#CDCDCD] text-red-500 font-bold">
                                   {order.status === OrderStatus.EXCHANGE_COMPLETE ? '교환완료' : '교환신청'}
                               </td>
                               <td className="border-r border-[#CDCDCD]">-</td>
                               <td className="border-r border-[#CDCDCD]">-</td>
                               <td className="">PC</td>
