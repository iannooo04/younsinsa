  ...
  const fetchOrders = useCallback(async () => {
      ...
      try {
          const res = await getOrdersAction({
              page,
              limit,
              status: [OrderStatus.REFUND_REQUEST, OrderStatus.REFUND_COMPLETE],
              keyword,
              searchType,
            ...
            <div className="flex items-center text-xs border-b border-gray-200">
             ...
             <div className="flex items-center text-xs">
                ...
                <div className="flex-1 p-3 flex items-center gap-2">
                     <Select defaultValue="refund_request_date">
                        <SelectTrigger className="w-24 h-7 text-[11px] border-gray-300">
                            <SelectValue placeholder="환불접수일" />
                        </SelectTrigger>
                        <SelectContent>
          ...
          <div className="flex items-center gap-1 text-xs">
              ...
              <Select defaultValue="status">
                  ...
                  <SelectContent>
                      ...
                      <SelectItem value="payment_complete">결제완료</SelectItem>
                      <SelectItem value="product_preparing">상품준비중</SelectItem>
                      <SelectItem value="delivered">배송완료</SelectItem>
                      <SelectItem value="purchase_confirm">구매확정</SelectItem>
                      <SelectItem value="return_request">반품접수</SelectItem>
                      <SelectItem value="refund_hold">환불보류</SelectItem>
                  </SelectContent>
              </Select>
          ...
          <table className="w-full text-xs text-center border-collapse table-fixed min-w-[2000px]">
              ...
              <thead className="bg-[#BDBDBD] text-white font-normal">
                  <tr className="h-10">
                      ...
                      <th className="border-r border-[#CDCDCD] break-keep leading-tight px-1">환불신청일<br/>(환불접수일)</th>
                      <th className="border-r border-[#CDCDCD]">환불완료일시</th>
                      ...
                      <th className="border-r border-[#CDCDCD]">환불수단</th>
                      <th className="border-r border-[#CDCDCD]">환불처리</th>
              ...
              <tbody className="text-gray-600 bg-white">
                 {loading ? (
                    <tr>
                        <td colSpan={22} className="py-10 border-b border-gray-200 text-center text-sm">로딩중...</td>
                    </tr>
                  ) : orders.length === 0 ? (
                      <tr>
                          <td colSpan={22} className="py-10 border-b border-gray-200 text-center text-sm">
                              검색된 주문이 없습니다.
                          </td>
                      </tr>
                  ) : (
                      orders.map((order, idx) => (
                          <tr key={order.id} className="border-b border-gray-200 hover:bg-gray-50 h-8">
                               ...
                               <td className="border-r border-[#CDCDCD]">{order.statusUpdatedAt && order.status === OrderStatus.REFUND_REQUEST ? format(new Date(order.statusUpdatedAt), "yyyy-MM-dd HH:mm") : '-'}</td>
                               <td className="border-r border-[#CDCDCD]">{order.status === OrderStatus.REFUND_COMPLETE ? (order.statusUpdatedAt ? format(new Date(order.statusUpdatedAt), "yyyy-MM-dd HH:mm") : '-') : '-'}</td>
                               ...
                               <td className="border-r border-[#CDCDCD] text-red-500 font-bold">
                                   {order.status === OrderStatus.REFUND_COMPLETE ? '환불완료' : '환불신청'}
                               </td>
                               <td className="border-r border-[#CDCDCD]">-</td>
                               <td className="border-r border-[#CDCDCD]">-</td>
                               <td className="border-r border-[#CDCDCD]">-</td>
              ...
              <div className="space-y-4">
                   <div>
                       ...
                       <div className="pl-0 text-xs text-gray-500 space-y-1">
                           <p>· 환불 리스트의 주문상태는 추가할 수 없으나, "기본설정{">"}주문정책{">"}주문상태 설정" 메뉴 내 "환불 상태 설정" 항목에서 운영자가 관리자페이지와 쇼핑몰페이지에 노출될 주문상태명을 각각 수정할 수 있습니다.</p>
                           <p>· 수정된 주문상태명은 저장 즉시 관리자페이지와 쇼핑몰페이지에 반영되어 수정된 정보로 노출됩니다.</p>
                           <p>· 환불 리스트의 "환불접수, 환불완료" 상태는 운영자가 사용여부를 설정할 수 없으나,</p>
                           <p className="pl-2">"환불보류" 상태는 환불 상태 설정 메뉴의 "사용설정" 항목에 체크박스가 체크된 상태에만 관리자페이지와 쇼핑몰페이지에 노출되며, 체크가 되지않은 경우 노출되지 않습니다.</p>
                       </div>
                  </div>
                  ...
                  <div>
                       ...
                       <div className="pl-0 text-xs text-gray-500 space-y-1">
                           <p>· 환불 리스트 내 주문은 상품별로 이전 단계인 "배송완료/구매확정/반품접수" 또는 "환불접수/환불보류"의 환불상태 중 선택하여 변경할 수 있습니다,</p>
                           <p>· "기본설정{">"}주문정책{">"}주문상태 설정"에서 "배송 상태 설정 / 구매확정 상태 설정" 항목에 추가된 주문상태가 있는 경우, 해당 주문상태로도 변경할 수 있습니다.</p>
                           <p>· 주문리스트에서 환불접수 또는 환불보류 상태의 주문을 환불완료 상태로 변경이 불가능합니다.</p>
                           <p className="text-red-500 font-bold">(환불완료 상태로의 변경은 "환불처리" 버튼을 이용 "환불 상세정보" 페이지에서 가능합니다.)</p>
                       </div>
                  </div>
              </div>
           </div>
